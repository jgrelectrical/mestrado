function out = quantil_k(ordenado,original,q,k)

serie_intervalo=ordenado(:,2);
sinal_original=original(:,2);
serie_original=original(:,2);
...
for i = 1:(n-k)% construindo a matriz(ligando os quantis(nos))
    ...
    matrizadj(localiza(i,1),localiza(i+k,1)) = matrizadj(localiza(i,1),localiza(i+k,1)) + 1;
       contador = contador+1;
end
